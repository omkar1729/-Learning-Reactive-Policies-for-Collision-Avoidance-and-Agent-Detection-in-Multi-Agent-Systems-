...
class Scenario(BaseScenario):
    ...
    def reset_world(self, world):
        ...
        for i, landmark in enumerate(world.landmarks):
            landmark.color = np.array([0.0, 0.0, 0.0])
        ...
        for i, landmark in enumerate(world.landmarks):
            landmark.state.p_pos = np.random.uniform(-0, +0, world.dim_p)
    ...
    def agent_reward(self, agent, world):
        ...
        dst = -np.sqrt(np.sum(np.square(agent.state.p_pos - agent.goal_a.state.p_pos)))
        return dst
    ...
    def adversary_reward(self, agent, world):
        ...
        nearest_agent = world.agents[np.argmin(agent_dist)]
