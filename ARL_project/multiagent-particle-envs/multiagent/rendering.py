...
class Viewer(object):
    ...
    # Convenience
    def draw_circle(self, radius=10, res=30, filled=False, **attrs):
        geom = make_circle(radius=radius, res=res, filled=filled)
        _add_attrs(geom, attrs)
        self.add_onetime(geom)
        return geom

    def draw_wall(self, radius=10, res=4, filled=False, **attrs):
        geom = make_wall(radius=radius, res=res, filled=filled)
        _add_attrs(geom, attrs)
        self.add_onetime(geom)
        return geom
...
def make_circle(radius=10, res=80, filled=True):
    points = []
    for i in range(res):
        ang = 2 * math.pi * (i) / res
